 ...
 *   'PR_TAG': string,
 *   'PR_TYPE': string,
 *   'highlightOne': function (string, string, number|boolean),
 *   'highlight': function (?function, ?(HTMLElement|HTMLDocument))
 * }}
 * @const
 */
var PR;
...
/**
 * Split {@code highlight} into multiple timeouts so as not to interfere with
 * UI events.
 * If set to {@code false}, {@code highlight()} is synchronous.
 */
...
(function () {
   ...
   *     or the 1-indexed number of the first line in sourceCodeHtml.
   */
  function $highlightOne(sourceCodeHtml, opt_langExtension, opt_numberLines) {
    /** @type{number|boolean} */
    ...
    *   Defaults to {@code document.body}.
    */
  function $highlight(opt_whenDone, opt_root) {
    ...
    var highlightRe = /\bprettyprint\b/;
    var highlightedRe = /\bprettyprinted\b/;
    ...
    function doWork() {
      ...
      for (; k < elements.length && clock['now']() < endTime; k++) {
        ...
        if ((attrs !== EMPTY || highlightRe.test(className))
            // Don't redo this if we've already done it.
            // This allows recalling pretty print to just prettyprint elements
            // that have been added to the page since last call.
            && !highlightedRe.test(className)) {
          ...
          for (var p = cs.parentNode; p; p = p.parentNode) {
            ...
            if (preCodeXmpRe.test(tn)
                && p.className && highlightRe.test(p.className)) {
              nested = true;
              break;
            }
          }
          if (!nested) {
            ...
            applyDecorator(highlightingJob);
          }
        }
      }
